...
exports.getComposersByGenre = async (req, res) => {
  ...
  mysql_pool.getConnection(function (err, connection) {
    connection.query(
      `SELECT DISTINCT user.id,user.uid,user.firstName,user.lastName,user.username,user.email,user.isPublisher,specializationTag.tagID,tag.tagName,userProfile.profilePicPath,content.audioFilename,content.audioFilepath 
